...
public class UnifiedScheduleDto { //  for (구글 일정 + 내부 일정) 하나의 형태로 묶기
  ...
  // 우리 서비스 일정 → DTO
  public static UnifiedScheduleDto fromService(Schedule s) {
    return UnifiedScheduleDto.builder()
        ...
        .isGrouped(false) // 필요하면 s.getEvent() != null 로 변경
        .groupName(null)  // 그룹 일정이면 s.getEvent().getGroup().getName()
        .meetingType(null) // 아직 MeetingType이 없다면 null 처리
